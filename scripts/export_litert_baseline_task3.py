...
def main() -> None:
    state_path = ROOT / "models" / "baseline_task3.pt"
    scaling_path = ROOT / "models" / "baseline_task3_scaling.npz"
    out_path = ROOT / "models" / "baseline_task3.tflite"
    firmware_dir = ROOT / "firmware" / "vlp_serial"
    firmware_path = firmware_dir / "vlp_model.tflite"

    # model = BaselineMLP(9)
    model = ImprovedMLP(
        layers=[36, 55, 32, 15, 2],
        activation="sigmoid",
        last_act="relu",
        layer_norm=False,
    )
